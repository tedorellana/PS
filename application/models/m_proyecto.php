...
class M_proyecto extends CI_Model{
	...
	public function traer_proyectos(){
		$this->db->select('t_proyecto.idProyecto,t_proyecto.fk_idEstado,t_proyecto.nombProyecto,t_persona.nombPersona,t_proyecto.dirProyecto,t_estado.nombEstado,t_proyecto.respaldo');
		$this->db->from('t_proyecto');
	...
	public function getProyectosFaltante($faltante){
		$this->db->select('*');
		$this->db->where('idProyecto', $faltante);
		$this->db->from('t_proyecto');
		$query = $this->db->get();
		if(empty($query)){
	    	return false;
	...
	public function marcaComoRespaldado($id){
		$this->db->where('idProyecto', $id);
		$this->db->update('t_proyecto', array('respaldo' => 'SI' )); 
	}
